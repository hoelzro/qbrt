...
class CTarget
	...
	# Exclude these files from compilation
	def exclude_files(*files)
		@excl_src.concat(files)
	end
	...
	def dependencies( obj )
		src = obj_to_src( obj )
		deps = [ src ]
		headers = @dependencies.headers( src, @inc )
		deps.concat( headers )
		return deps
	end
	...
	def obj_to_src( obj )
		src = obj.sub( /^#{obj_dir}\//, '' )
		cpp = src.sub( '.o', '.cpp' )
		return cpp if @src.include? cpp
		c = src.sub( '.o', '.c' )
		return c if @src.include? c
		raise "no source file for #{obj} in #{@src.to_s}"
	end
...
class CProject
	...
	def dependencies( object )
		...
		#puts "target = #{t.name}, object = #{object}\n"
		deps = t.dependencies( object )
		#puts "\tdeps = #{deps}"
		return deps
	end
...
class CCompiler
	...
	def compile( object, target )
		...
		inc_flags = include_flags( target.inc )
		src = target.obj_to_src( object )
		cmd = "#{@cc} -c #{cc_flags} #{inc_flags} -o #{object} #{src}"
		exec cmd
	end
	...
	def link( target )
		...
		lib_flags = lib_flags( target.lib )
		cmd = "#{@cc} -o #{target.name} #{lib_flags} #{objs}"
#print cmd
		exec cmd
	end
